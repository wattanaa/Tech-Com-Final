...
export function ToastProvider({ children }: { children: ReactNode }) {
  ...
  return (
    <ToastContext.Provider value={value}>
      {children}
      <div
        className="fixed inset-x-4 bottom-4 z-[100] flex flex-col items-stretch gap-2 sm:inset-x-auto sm:right-4 sm:items-end"
        aria-live="polite"
      >
        {toasts.map((t) => (
          <div
            ...
            role="status"
            className={cn(
              'glass-strong flex items-start gap-2.5 rounded-lg px-4 py-3 text-sm shadow-float sm:w-auto sm:max-w-sm',
            )}
          >
            {t.variant === 'success' ? (
